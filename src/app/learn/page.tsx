...
import { BookOpen, Clock, Search, ArrowRight, Sparkles } from 'lucide-react';

export default function LearnCatalogPage() {
  const [search, setSearch] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('All');

  const categories = ['All', 'Mutual Funds & Investing', 'Financial Metrics', 'Foundations of Finance', 'Debt & Loans', 'Savings & Banking'];

  const filteredArticles = LEARN_ARTICLES.filter((art) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8 animate-fade-in">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 dark:bg-sky-950/60 border border-sky-200 dark:border-sky-800 text-xs font-bold text-sky-600 dark:text-sky-400">
          ...
          <span>Finance Knowledge Center</span>
        </div>
        <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
          Learn Personal Finance
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
          In-depth, jargon-free guides on SIPs, compound interest, net worth, EMIs, and debt elimination strategies.
        </p>
      </div>
      ...
      <div className="max-w-xl mx-auto space-y-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search guides on SIP, CAGR, XIRR, EMI, Net Worth..."
        ...
        {filteredArticles.map((article) => (
          ...
          >
            <div className="space-y-3">
              ...
              <h3 className="text-base font-black text-slate-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors leading-snug">
                {article.title}
              </h3>
